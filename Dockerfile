...
# PostHog (analytics produit) : token PUBLIC projet + host. Sans ces build-args,
# `instrumentation-client.ts` voit un token vide et n'initialise jamais PostHog
# (aucun event ingéré, alors même que le reverse-proxy /ingest répond).
